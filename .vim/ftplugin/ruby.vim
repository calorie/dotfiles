...
"<F5>でRubyスクリプトを保存後シンタックスチェックし、エラーをQuickFixに表示
map <F5> :w<CR>:make -c %<CR>:cw<CR>:cfirst<CR>zz
nmap <S-F5> :ccl<CR>

"<F6>でバッファのRubyスクリプトを実行し、結果をプレビュー表示
vmap <F6> :call Ruby_eval_vsplit()<CR>
nmap <F6> mzggVG<F10>`z
map  <S-F6> :pc<CR>
...
"<F5>でRubyスクリプトを保存後シンタックスチェックし、エラーをQuickFixに表示
nmap <F5> ;w<CR>;make -c %<CR>;cw<CR>;cfirst<CR>zz
nmap <S-F5> ;ccl<CR>
